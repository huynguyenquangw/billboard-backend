...
export class BillboardService {
    constructor(
        @InjectRepository(BillboardEnity) private billboardRepo: Repository<BillboardEnity>,
        @InjectRepository(DistrictEntity) private districtRepo: Repository<DistrictEntity>,
        @InjectRepository(UserEntity) private userRepo: Repository<UserEntity>
    ){}


    async createbillBoard(billboardDto : BillboardDto): Promise<BillboardEnity>{
        const district1 = await this.districtRepo.findOne({where: {id: billboardDto.districtId}});
        const user1= await this.userRepo.findOne({where: {id: billboardDto.userId}});
